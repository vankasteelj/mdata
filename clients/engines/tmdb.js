module.exports = class TMDB {
  ...
  movie(ids) { // imdb, tmdb
    return this.matchId('movie', ids, 'movie_results').then(tmdb => this.client.movie.images({movie_id: tmdb})).then(this.extractBestForShowMovie.bind(this))
  }

  show(ids) { // imdb, tvdb, tmdb
    return this.matchId('show', ids, 'tv_results').then(tmdb => this.client.tv.images({tv_id: tmdb})).then(this.extractBestForShowMovie.bind(this))
  }

  season(ids, season) { // imdb, tvdb, tmdb
    return this.matchId('season', ids, 'tv_season_results').then(tmdb => this.client.tv.season.images({tv_id: tmdb, season: season})).then(this.extractBestForSeason.bind(this))
  }

  episode(ids, season, episode) { // imdb, tvdb, tmdb
    return this.matchId('episode', ids, 'tv_episode_results').then(tmdb => this.client.tv.episode.images({tv_id: tmdb, season: season, episode: episode})).then(this.extractBestForEpisode.bind(this))
  }
